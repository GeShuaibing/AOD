...
from .Function_evc import Encoding, Mean, Mlp, GroupNorm, ConvBlock, DropPath
...
class UP_EVC(nn.Module):
    def __init__(self, cin, cout):
        super(UP_EVC, self).__init__()
        self.reduce = BaseConv(
            cin, cout, 1, 1
        )
        self.up = nn.Upsample(scale_factor=2, mode="nearest")  # scale_factor=2, mode="nearest"
        self.evc = EVCBlock(cout, cout, channel_ratio=4, base_channel=16)

    def forward(self, x):
        return self.evc(self.up(self.reduce(x)))
