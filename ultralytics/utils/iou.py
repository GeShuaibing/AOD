...
class IoU_Cal:
    ...
    iou_mean = 1.
    monotonous = False #v1:none v2:true v3:false
    ...
    def __init__(self, pred, target):
        ...
        self._fget = {
            ...
            # IoU
            'iou': lambda: 1 - self.s_inter / self.s_union    # iou loss
        }
        self._update(self)
    ...
    def _scaled_loss(self, loss, alpha=1.9, delta=3,version=None):
        self.monotonous = version
    ...
    def IoU(cls, pred, target, self=None):
        ...
        return self.iou
    @classmethod
    def WIoUv3(cls, pred, target, self=None):
        self = self if self else cls(pred, target)
        dist = torch.exp(self.l2_center / self.l2_box.detach())
        return self._scaled_loss((dist * self.iou),version=False)
    @classmethod
    def WIoUv2(cls, pred, target, self=None):
        # print("wiouv2")
        self = self if self else cls(pred, target)
        dist = torch.exp(self.l2_center / self.l2_box.detach())
        return self._scaled_loss((dist * self.iou), version=True)
    @classmethod
    def WIoUv1(cls, pred, target, self=None):
        self = self if self else cls(pred, target)
        dist = torch.exp(self.l2_center / self.l2_box.detach())
    ...
    @classmethod
    def FEIoU(cls,pred, target, self=None,eps=1e-7,gamma=0.5):
        self = self if self else cls(pred, target)
        penalty = self.l2_center / self.l2_box.detach() \
                  + torch.square(self.d_center / self.wh_box).sum(dim=-1)
        return torch.pow((1-self.iou+eps),gamma)*self._scaled_loss(self.iou + penalty)
    @classmethod
